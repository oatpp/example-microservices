...
void Runner::run(v_word16 port, const std::shared_ptr<NetworkVirtualInterface>& interface) {
  ...
  if(port > 0) {
    ...
    OATPP_LOGI("user-service", "server is listening on port '%hu'", port);
  ...
  if(interface) {
    ...
    OATPP_LOGI("user-service", "server is listening on virtual interface '%s'", interface->getName()->getData());
